...
const signInEpic = (action$: ActionsObservable<any>, store$: StateObservable<Store>) =>
    action$.pipe(
        ...
        mergeMap((action) => {
            ...
            let appsettings = window.localStorage.getItem(`${location.hostname}_auth-app_appsettings`);
...
const reSignInEpic = (action$: ActionsObservable<any>, store$: StateObservable<Store>) =>
    action$.pipe(
        ...
        mergeMap((action) => {
            ...
            const { AUTH_PROTOCOL, AUTH_HOST, AUTH_PORT, AUTH_BASEPATH } = JSON.parse(window.localStorage.getItem(`${location.hostname}_auth-app_appsettings`));
            return new GraphQLObservable({ protocol: AUTH_PROTOCOL, host: AUTH_HOST, port: AUTH_PORT, basePath: AUTH_BASEPATH, method: 'POST', body: body, prop: 'signin' });
        }),
        map((payload) => {
            if (payload.isValid) {
